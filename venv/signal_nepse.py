...
# In[2]:
#os.chdir('d:/')<--changed by prasanna
...
def bollinger_bands(df):
    ...
    data['std'] = data['adjclose'].rolling(window=20, min_periods=20).std()
    data['mid band'] = data['adjclose'].rolling(window=20, min_periods=20).mean()
...
def signal_generation(data, method):
    ...
    for i in range(period, len(df)):
        ...
        # condition 4
        if (df['adjclose'][i] > df['upper band'][i]) and \
                (df['cumsum'][i] == 0):

            for j in range(i, i - period, -1):

                # condition 2
                if (np.abs(df['mid band'][j] - df['adjclose'][j]) < alpha) and \
            ...
            if moveon == True:
                ...
                for k in range(j, i - period, -1):
                    ...
                    # condition 1
                    if (np.abs(df['lower band'][k] - df['adjclose'][k]) < alpha):
                        threshold = df['adjclose'][k]
                        moveon = True
                        break
            ...
            if moveon == True:
                ...
                for l in range(k, i - period, -1):
                    ...
                    if (df['mid band'][l] < df['adjclose'][l]):
                        moveon = True
                        break
            ...
            if moveon == True:
                ...
                for m in range(i, j, -1):
                    ...
                    # condition 3
                    if (df['adjclose'][m] - df['lower band'][m] < alpha) and \
                            (df['adjclose'][m] > df['lower band'][m]) and \
                            (df['adjclose'][m] < threshold):
                        df.at[i, 'signals'] = 1
...
def plot(new):
    ...
    ax.plot(newbie['adjclose'], label='price')
    ...
    ax.plot(newbie['adjclose'][newbie['signals'] == 1], marker='^', markersize=12, \
            lw=0, c='g', label='LONG')
    ax.plot(newbie['adjclose'][newbie['signals'] == -1], marker='v', markersize=12, \
    ...
    ax.plot(newbie['adjclose'][pd.to_datetime(new['date'].iloc[indexlist])], \
